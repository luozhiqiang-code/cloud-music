...
const handleDeleteSong = (state, song) => {
  ...
  const fpIndex = findIndex(song, playList.toJS());
  const newPlayList = playList.splice(fpIndex, 1); // 直接在List上删除
  const sequencePlayList = state.get("sequencePlayList");
  const fsIndex = findIndex(song, sequencePlayList.toJS());
  const newSequencePlayList = sequencePlayList.splice(fsIndex, 1); // 直接在List上删除
...
const handleInsertSong = (state, song) => {
  ...
  //上一步添加歌曲是为了让歌曲在下一曲播放，
  // 如果列表中已经存在要添加的歌，暂且称它oldSong，应该把以前的删掉

  if (fpIndex > -1) {
    // 如果oldSong的索引比目前播放歌曲的索引小，那么删除它，同时当前index要减一
    if (currentIndex > fpIndex) {
      playList.splice(fpIndex, 1);
      currentIndex--;
    } else {
      // 否则直接删掉oldSong
      let a = playList.splice(fpIndex + 1, 1);
    }
  }
  // 同理，处理sequenceList
